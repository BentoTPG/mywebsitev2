...
const Navbar = () => {
  ...
  const linkClass = (path: string) =>
    `text-2xl flex gap-x-2 p-4 transition rounded-t-lg ${
      pathname === path ? "shadow-[4px_0_10px_rgba(0,0,0,0.2)] pageandpoint font-semibold"
      : "hover:bg-gray-700 dark:hover:bg-black"
    }`;
  return(
    <nav className="w-screen md:max-w-[1139px] transition-all duration-300 md:ml-67 h-16 mx-auto mt-5 compo shadow-md fixed bottom-0 md:top-0 z-50 flex justify-center md:justify-between opacity-95 md:opacity-100 items-center text-left  rounded-t-2xl font-[family-name:var(--font-vt-323)] ">
